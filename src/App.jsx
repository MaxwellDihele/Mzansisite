...
const PEOPLE = [
  ...
  { id:8,  name:"Thabo Dlamini",    handle:"@thabo_d",       avatar:"https://picsum.photos/seed/thabo8/200/200",    role:"Photographer",      industry:"Photography",       location:"Pretoria, ZA",     bio:"Documentary photographer. Prints for sale — DM to order 📸",       followers:7200,    following:388, projects:29,  verified:false, isFollowing:true,  portfolio:["https://picsum.photos/seed/t8a/300/300","https://picsum.photos/seed/t8b/300/300","https://picsum.photos/seed/t8c/300/300","https://picsum.photos/seed/t8d/300/300"] },
  ...
  { id:10, name:"Trevor Noah",      handle:"@trevornoah",    avatar:"https://picsum.photos/seed/trevor10/200/200",  role:"Comedian",          industry:"Acting & Performance",location:"New York / SA",    bio:"Comedian, author, host. All links here 👇",                                   followers:8900000, following:420, projects:55,  verified:true,  isFollowing:true,  portfolio:["https://picsum.photos/seed/tr10a/300/300","https://picsum.photos/seed/tr10b/300/300","https://picsum.photos/seed/tr10c/300/300","https://picsum.photos/seed/tr10d/300/300"] },
...
function AuthGateModal({ reason, onClose, onGoLogin, T }) {
  const msgs = {
    follow:    { icon:"person_add", title:"Sign in to follow", body:"Create an account or sign in to follow people and build your network." },
...
/* ─── SOCIAL LINK PREVIEW MODAL ─── */
function SocialLinkPreviewModal({ link, onClose, T }) {
  const plat = getPlatform(link.platformId);
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.65)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:1000, backdropFilter:"blur(8px)" }} onClick={onClose}>
      <div style={{ background:T.modalBg, borderRadius:"24px 24px 0 0", padding:"0 0 36px", width:"100%", maxWidth:"480px", border:`1px solid ${T.cardBorder}`, boxShadow:"0 -16px 60px rgba(0,0,0,0.3)" }} onClick={e=>e.stopPropagation()}>
        {/* Handle bar */}
        <div style={{ display:"flex", justifyContent:"center", padding:"12px 0 4px" }}>
          <div style={{ width:"40px", height:"4px", background:T.handleBar, borderRadius:"2px" }} />
        </div>
        {/* Header */}
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 20px 14px", borderBottom:`1px solid ${T.divider}` }}>
          <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
            <div style={{ width:"44px", height:"44px", borderRadius:"12px", background:plat.gradient, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", boxShadow:`0 4px 14px ${plat.color}40` }}>{plat.icon}</div>
            <div>
              <div style={{ fontSize:"16px", fontWeight:"800", color:T.white }}>{link.label}</div>
              <div style={{ fontSize:"12px", color:T.subText }}>{link.handle}</div>
            </div>
          </div>
          <button onClick={onClose} style={{ background:T.inputBg, border:`1px solid ${T.inputBorder}`, borderRadius:"50%", width:"32px", height:"32px", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
            <MI name="close" size={17} color={T.subText} />
          </button>
        </div>
        {/* Preview iframe */}
        <div style={{ margin:"16px 20px", borderRadius:"16px", overflow:"hidden", border:`1px solid ${T.cardBorder}`, background:T.sectionBg, height:"260px", position:"relative" }}>
          <iframe
            src={link.url}
            title={link.label}
            ...
            loading="lazy"
          />
          {/* Overlay message for sites that block embedding */}
          <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"8px", pointerEvents:"none", background:"transparent" }}>
            <div style={{ background:"rgba(0,0,0,0.55)", backdropFilter:"blur(6px)", borderRadius:"12px", padding:"10px 16px", display:"flex", alignItems:"center", gap:"8px" }}>
              <MI name="info_outline" size={15} color="rgba(255,255,255,0.7)" />
              <span style={{ fontSize:"11px", color:"rgba(255,255,255,0.7)" }}>Preview may be limited by the platform</span>
            </div>
          </div>
        </div>
        {/* URL display */}
        <div style={{ margin:"0 20px 16px", padding:"10px 14px", background:T.sectionBg, borderRadius:"10px", border:`1px solid ${T.cardBorder}`, display:"flex", alignItems:"center", gap:"8px", overflow:"hidden" }}>
          <MI name="link" size={15} color={T.subText} style={{ flexShrink:0 }} />
          <span style={{ fontSize:"12px", color:T.subText, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", flex:1 }}>{link.url}</span>
        </div>
        {/* Open button */}
        <div style={{ padding:"0 20px" }}>
          <a href={link.url} target="_blank" rel="noreferrer" style={{ textDecoration:"none", display:"block" }}>
            <button style={{ width:"100%", padding:"14px", background:`linear-gradient(135deg,${plat.color},${plat.color}cc)`, border:"none", borderRadius:"14px", color:"#fff", fontSize:"15px", fontWeight:"700", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:"9px", boxShadow:`0 4px 18px ${plat.color}40` }}>
              <MI name="open_in_new" size={18} color="#fff" /> Open {link.label}
            </button>
          </a>
        </div>
      </div>
...
function MiniProfile({ person, onBack, onAuthGate, isLoggedIn, T }) {
  ...
  return (
    <div style={{ background:T.bodyBg, minHeight:"100vh", fontFamily:"'DM Sans','Segoe UI',sans-serif", color:T.text }}>
      {/* Sub-nav: back + person name + home button */}
      <div style={{ display:"flex", alignItems:"center", gap:"10px", padding:"10px 14px", background:T.navBg, backdropFilter:"blur(16px)", borderBottom:`1px solid ${T.navBorder}`, position:"sticky", top:"58px", zIndex:190 }}>
        <button onClick={onBack} style={{ background:T.inputBg, border:`1px solid ${T.inputBorder}`, borderRadius:"50%", width:"34px", height:"34px", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", flexShrink:0 }}>
          <MI name="arrow_back" size={18} color={T.text} />
        </button>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontSize:"14px", fontWeight:"800", color:T.white, fontFamily:"'Playfair Display',Georgia,serif", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", display:"flex", alignItems:"center", gap:"5px" }}>
            {person.name}{person.verified&&<MI name="verified" size={14} color="#2a8a5a" />}
          </div>
          <div style={{ fontSize:"11px", color:T.subText }}>{person.handle}</div>
        </div>
        <button onClick={onBack} title="Back to home"
          style={{ background:"linear-gradient(135deg,#2a8a5a,#1e6b44)", border:"none", borderRadius:"20px", padding:"6px 12px", display:"flex", alignItems:"center", gap:"5px", cursor:"pointer", flexShrink:0 }}>
          <MI name="home" size={15} color="#fff" />
          <span style={{ fontSize:"11px", fontWeight:"700", color:"#fff" }}>Home</span>
        </button>
      </div>
      ...
      <div style={{ padding:"20px 16px 60px", maxWidth:"480px", margin:"0 auto" }}>
        ...
        <div style={{ background:T.cardBg, borderRadius:"22px", padding:"22px", marginBottom:"14px", border:`1px solid ${T.cardBorder}`, boxShadow:T.cardShadow }}>
          ...
          <div style={{ display:"flex", paddingTop:"14px", borderTop:`1px solid ${T.divider}` }}>
            {[{n:fmtN(followerCount),l:"Followers",gate:"followers"},{n:fmtN(person.following),l:"Following",gate:"following"},{n:String(person.portfolio.length),l:"Portfolio",gate:null}].map((s,i)=>(
              <button key={s.l} onClick={()=>{ if(s.gate && !isLoggedIn){ onAuthGate(s.gate); } else if(s.l==="Portfolio"){ setActiveTab("portfolio"); } }} disabled={!s.gate&&s.l!=="Portfolio"}
                style={{ flex:1, background:"none", border:"none", cursor:"pointer", padding:"8px 0", borderRight:i<2?`1px solid ${T.divider}`:"none", borderRadius:"8px" }}>
                <div style={{ fontSize:"16px", fontWeight:"800", color:T.white }}>{s.n}</div>
                <div style={{ fontSize:"11px", color:s.gate||s.l==="Portfolio"?"#2a8a5a":T.subText, display:"flex", alignItems:"center", justifyContent:"center", gap:"2px" }}>
                  {s.l}{(s.gate||s.l==="Portfolio")&&<MI name="arrow_forward" size={11} color="#2a8a5a" />}
                </div>
              </button>
            ))}
          </div>
        ...
        {/* Links tab — click opens preview modal */}
        {activeTab==="links" && (
            ...
            {subLinks.map(link=>{
              ...
              return (
                <button key={link.uid} onClick={()=>setPreviewLink(link)}
                  style={{ textDecoration:"none", display:"block", width:"100%", background:"none", border:"none", padding:0, cursor:"pointer", textAlign:"left" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:"14px", padding:"16px 18px", borderRadius:"16px", background:T.linkCard, border:`1px solid ${T.linkBorder}`, boxShadow:T.dark?"none":"0 1px 8px rgba(30,40,80,0.06)" }}>
                    ...
                    <MI name="open_in_new" size={18} color={plat.color} />
                  </div>
                </button>
              );
            })}
        ...
        {/* Social Link Preview Modal */}
        {previewLink && (
          <SocialLinkPreviewModal link={previewLink} onClose={()=>setPreviewLink(null)} T={T} />
        )}
      </div>
    </div>
  );
...
function SearchPage({ onViewProfile, onGoLogin, T }) {
  ...
  const hasInput = query.trim().length > 0 || industryFilter !== "All";
  ...
  const handleQueryChange = (e) => { setQuery(e.target.value); setVisibleCount(6); };
  const handleFilterChange = (ind) => { setIndustryFilter(ind); setVisibleCount(6); };

  const visibleResults = results.slice(0, visibleCount);
  const hasMore = visibleCount < results.length;

  const FEATURES = [
    { icon:"link", color:"#2a8a5a", label:"All Your Links", desc:"Add every social — Instagram, TikTok, Facebook & more" },
    { icon:"chat", color:"#25D366", label:"WhatsApp Link", desc:"Add your WhatsApp so anyone can message you directly" },
    { icon:"storefront", color:"#f59e0b", label:"Sell Anything", desc:"Products, services, bookings — direct via WhatsApp" },
    { icon:"campaign", color:"#e91e8c", label:"Creators", desc:"Grow your audience from one shareable link" },
    { icon:"build", color:"#0ea5e9", label:"Services", desc:"Freelancers, tradies, tutors & consultants" },
    { icon:"people", color:"#7c3aed", label:"Everyone", desc:"Casual users, hustlers, professionals — all welcome" },
  ];

  const STATS = [
    { n:"1 Link", l:"All Socials" },
    { n:"WhatsApp", l:"Friendly" },
    { n:"Sell &", l:"Connect" },
    { n:"Free", l:"To Start" },
  ];

  return (
    <div style={{ padding:"16px 16px 60px", maxWidth:"480px", margin:"0 auto" }}>

      {/* Search bar — sticky feel, always top */}
      <div style={{ position:"relative", marginBottom:"14px", marginTop:"8px" }}>
        <MI name="search" size={20} color={hasInput?"#2a8a5a":T.subText} style={{ position:"absolute", left:"14px", top:"50%", transform:"translateY(-50%)", transition:"color .15s" }} />
        <input value={query} onChange={handleQueryChange} placeholder="Search by name, handle or what they do…" autoComplete="off"
          style={{ width:"100%", background:T.cardBg, border:`1.5px solid ${hasInput?"#2a8a5a":T.inputBorder}`, borderRadius:"16px", padding:"14px 44px 14px 44px", fontSize:"14px", color:T.text, outline:"none", boxSizing:"border-box", fontFamily:"inherit", boxShadow:T.cardShadow, transition:"border-color .15s" }} />
        {query && <button onClick={()=>{ setQuery(""); setVisibleCount(6); }} style={{ position:"absolute", right:"12px", top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer" }}><MI name="close" size={18} color={T.subText} /></button>}
      </div>
      ...
      {/* ── EMPTY STATE: Platform intro ── */}
      {!hasInput && (
        <>
          {/* Hero */}
          <div style={{ textAlign:"center", padding:"20px 8px 24px" }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:"6px", background:"rgba(42,138,90,0.1)", border:"1px solid rgba(42,138,90,0.25)", borderRadius:"20px", padding:"4px 12px", marginBottom:"12px" }}>
              <MI name="bolt" size={13} color="#2a8a5a" />
              <span style={{ fontSize:"11px", fontWeight:"700", color:"#2a8a5a" }}>Smarter than a link in bio.</span>
            </div>
            <div style={{ fontSize:"26px", fontWeight:"900", color:T.white, fontFamily:"'Playfair Display',Georgia,serif", lineHeight:1.2, marginBottom:"10px" }}>
              All your links.<br/>One simple profile.
            </div>
            <div style={{ fontSize:"13px", color:T.subText, lineHeight:1.65, maxWidth:"300px", margin:"0 auto 20px" }}>
              Instagram, TikTok, Facebook, WhatsApp — stop sending people to different places. One link does it all.
            </div>
            <div style={{ display:"flex", gap:"10px", justifyContent:"center", flexWrap:"wrap" }}>
              <button onClick={onGoLogin} style={{ padding:"11px 22px", background:"linear-gradient(135deg,#2a8a5a,#1e6b44)", border:"none", borderRadius:"22px", color:"#fff", fontSize:"13px", fontWeight:"700", cursor:"pointer", boxShadow:"0 4px 16px rgba(42,138,90,0.35)", display:"inline-flex", alignItems:"center", gap:"7px" }}>
                <MI name="add_link" size={16} color="#fff" /> Get My Free Link
              </button>
              <button onClick={onGoLogin} style={{ padding:"11px 18px", background:"#25D366", border:"none", borderRadius:"22px", color:"#fff", fontSize:"13px", fontWeight:"700", cursor:"pointer", boxShadow:"0 4px 14px rgba(37,211,102,0.35)", display:"inline-flex", alignItems:"center", gap:"7px" }}>
                <MI name="chat" size={16} color="#fff" /> Share via WhatsApp
              </button>
            </div>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"8px", marginBottom:"22px" }}>
            {STATS.map(s=>(
              <div key={s.l} style={{ background:T.cardBg, border:`1px solid ${T.cardBorder}`, borderRadius:"14px", padding:"12px 6px", textAlign:"center" }}>
                <div style={{ fontSize:"16px", fontWeight:"900", color:T.white }}>{s.n}</div>
                <div style={{ fontSize:"10px", color:T.subText, marginTop:"2px", fontWeight:"600" }}>{s.l}</div>
              </div>
            ))}
          </div>

          {/* Category grid */}
          <div style={{ marginBottom:"6px", fontSize:"12px", fontWeight:"700", color:T.subText, letterSpacing:"0.05em", textTransform:"uppercase" }}>What can you do here?</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", marginBottom:"22px" }}>
            {FEATURES.map(f=>(
              <button key={f.label} onClick={()=>handleFilterChange(f.label)}
                style={{ background:T.cardBg, border:`1px solid ${T.cardBorder}`, borderRadius:"16px", padding:"16px 14px", textAlign:"left", cursor:"pointer", display:"flex", flexDirection:"column", gap:"8px", transition:"transform .12s, border-color .12s" }}
                onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.borderColor=f.color+"60";}}
                onMouseLeave={e=>{e.currentTarget.style.transform="none";e.currentTarget.style.borderColor=T.cardBorder;}}>
                <div style={{ width:"38px", height:"38px", borderRadius:"10px", background:f.color+"18", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  <MI name={f.icon} size={20} color={f.color} />
                </div>
                <div>
                  <div style={{ fontSize:"13px", fontWeight:"700", color:T.white, marginBottom:"2px" }}>{f.label}</div>
                  <div style={{ fontSize:"11px", color:T.subText, lineHeight:1.4 }}>{f.desc}</div>
                </div>
              </button>
            ))}
          </div>
          ...
          {/* How it works */}
          <div style={{ background:T.cardBg, border:`1px solid ${T.cardBorder}`, borderRadius:"18px", padding:"20px", marginBottom:"20px" }}>
            <div style={{ fontSize:"14px", fontWeight:"800", color:T.white, marginBottom:"14px" }}>Simpler than a link in bio</div>
            {[
              { icon:"add_link", color:"#2a8a5a", title:"Add all your socials", desc:"Instagram, TikTok, Facebook, WhatsApp, website — one place" },
              { icon:"share", color:"#25D366", title:"Share one link everywhere", desc:"Put it in your bio, send it on WhatsApp, print it anywhere" },
              { icon:"storefront", color:"#f59e0b", title:"Let people reach you instantly", desc:"They tap your WhatsApp link and you're already talking" },
            ].map((step,i)=>(
              <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:"12px", marginBottom:i<2?"14px":"0" }}>
                <div style={{ width:"34px", height:"34px", borderRadius:"10px", background:step.color+"18", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                  <MI name={step.icon} size={18} color={step.color} />
                </div>
                <div>
                  <div style={{ fontSize:"13px", fontWeight:"700", color:T.white }}>{step.title}</div>
                  <div style={{ fontSize:"12px", color:T.subText }}>{step.desc}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Sign-in CTA */}
          <div style={{ padding:"18px 20px", background:T.tagBg, borderRadius:"18px", border:`1px solid ${T.tagBorder}`, display:"flex", alignItems:"center", gap:"14px" }}>
            <MI name="workspace_premium" size={24} color="#2a8a5a" />
            <div style={{ flex:1 }}>
              <div style={{ fontSize:"13px", fontWeight:"700", color:T.white, marginBottom:"2px" }}>Get your free profile link</div>
              <div style={{ fontSize:"12px", color:T.subText }}>Your link in bio. All your socials. Start for free.</div>
            </div>
      ...
      {/* ── RESULTS: shown only after user has typed or filtered ── */}
      {hasInput && (
        <>
          <div style={{ fontSize:"12px", color:T.mutedText, marginBottom:"12px" }}>
            {results.length} profile{results.length!==1?"s":""} found
            {industryFilter!=="All" && <span> · <button onClick={()=>handleFilterChange("All")} style={{ background:"none", border:"none", color:"#2a8a5a", cursor:"pointer", fontSize:"12px", fontWeight:"600", padding:0 }}>Clear filter ×</button></span>}
          </div>
            ...
            <>
              ...
              {hasMore && (
                <button onClick={()=>setVisibleCount(c=>c+6)}
                  style={{ width:"100%", marginTop:"14px", padding:"13px", background:T.cardBg, border:`1px solid ${T.cardBorder}`, borderRadius:"14px", color:"#2a8a5a", fontSize:"13px", fontWeight:"700", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:"7px" }}
                  onMouseEnter={e=>{e.currentTarget.style.borderColor="#2a8a5a50";e.currentTarget.style.background=T.tagBg;}}
                  onMouseLeave={e=>{e.currentTarget.style.borderColor=T.cardBorder;e.currentTarget.style.background=T.cardBg;}}>
                  <MI name="expand_more" size={18} color="#2a8a5a" />
...
export default function ProfilePage() {
  ...
  const [profileData, setProfileData] = useState({
    ...
    location:"Johannesburg, ZA", bio:"📷 Photographer & content creator. All my links, bookings & WhatsApp right here 👇",
  ...
  /* ════ NAV ════ */
  const Nav = () => {
    const isOnProfile = page === "profile" && isLoggedIn && !viewingProfile;
    const isOnSearch  = page === "search" && !viewingProfile;
    ...
    return (
      <nav style={{
        ...
        {/* ── Main bar ── */}
        <div style={{ height:"58px", display:"flex", alignItems:"center", padding:"0 14px", gap:"8px" }}>

          {/* Left: Logo (taps home based on auth state) */}
          <button
            onClick={()=>{ setPage(isLoggedIn?"profile":"search"); setViewingProfile(null); }}
          ...
          {/* Center: contextual breadcrumb / page label */}
          <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden" }}>
            {viewingProfile && (
              <div style={{ display:"flex", alignItems:"center", gap:"6px", background:T.tagBg, border:`1px solid ${T.tagBorder}`, borderRadius:"20px", padding:"4px 12px 4px 8px", maxWidth:"180px" }}>
                <MI name="person" size={14} color="#2a8a5a" />
                <span style={{ fontSize:"12px", fontWeight:"700", color:"#2a8a5a", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{viewingProfile.name}</span>
              </div>
            )}
            {!viewingProfile && isOnSearch && (
              <div style={{ display:"flex", alignItems:"center", gap:"6px", background:T.tagBg, border:`1px solid ${T.tagBorder}`, borderRadius:"20px", padding:"4px 12px 4px 8px" }}>
                <MI name="search" size={14} color="#2a8a5a" />
                <span style={{ fontSize:"12px", fontWeight:"700", color:"#2a8a5a", letterSpacing:"0.01em" }}>Explore Profiles</span>
              </div>
            )}
            {!viewingProfile && isOnProfile && (
              <div style={{ display:"flex", alignItems:"center", gap:"6px", background:T.tagBg, border:`1px solid ${T.tagBorder}`, borderRadius:"20px", padding:"4px 12px 4px 8px", maxWidth:"200px" }}>
                <MI name="verified_user" size={14} color="#2a8a5a" />
                <span style={{ fontSize:"12px", fontWeight:"700", color:"#2a8a5a", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>My Profile</span>
              </div>
            )}
            {!viewingProfile && page === "login" && (
              <span style={{ fontSize:"13px", fontWeight:"700", color:T.subText }}>Sign In</span>
            )}
          </div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"6px", flexShrink:0 }}>
            ...
            {/* Search icon — always shown, active state when on search page */}
            <button
              onClick={()=>{ setViewingProfile(null); setPage("search"); }}
              title="Browse Talent"
              style={{
                background: isOnSearch ? "linear-gradient(135deg,#2a8a5a,#1e6b44)" : T.inputBg,
                border: isOnSearch ? "none" : `1px solid ${T.inputBorder}`,
                borderRadius:"10px", width:"36px", height:"36px", cursor:"pointer",
                display:"flex", alignItems:"center", justifyContent:"center",
                boxShadow: isOnSearch ? "0 2px 10px rgba(42,138,90,0.3)" : "none",
                transition:"all 0.15s",
              }}>
              <MI name="search" size={20} color={isOnSearch?"#fff":T.subText} />
            </button>

            {/* My Profile icon (logged-in) OR Sign In button (guest) */}
            {isLoggedIn ? (
              <button
                ...
                style={{
                  ...
                  outline: isOnProfile ? "2.5px solid #2a8a5a" : "2px solid transparent",
                  outlineOffset:"1px",
                  transition:"outline 0.15s",
                }}>
                <LazyAvatar src={profileData.avatar} alt="me" size={32} border="none" />
              </button>
            ) : (
              <button
                ...
                style={{
                  padding:"7px 14px",
                  background:"linear-gradient(135deg,#2a8a5a,#1e6b44)",
                  border:"none", borderRadius:"20px",
                  color:"#fff", fontSize:"12px", fontWeight:"700",
                  cursor:"pointer", display:"flex", alignItems:"center", gap:"5px",
                  boxShadow:"0 2px 10px rgba(42,138,90,0.3)",
                  whiteSpace:"nowrap",
                }}>
            ...
            {/* Hamburger — 3 lines, animated to X when open */}
            <button
              onClick={()=>setDrawerOpen(o=>!o)}
              title="Menu"
              style={{
                background: drawerOpen ? T.tagBg : T.inputBg,
                border: drawerOpen ? `1px solid ${T.tagBorder}` : `1px solid ${T.inputBorder}`,
                borderRadius:"10px", width:"36px", height:"36px",
                cursor:"pointer", display:"flex", flexDirection:"column",
                alignItems:"center", justifyContent:"center", gap:"4px",
                transition:"all 0.15s",
              }}>
              <div style={{ width:18, height:2, background: drawerOpen?"#2a8a5a":T.text, borderRadius:2, transform: drawerOpen?"rotate(45deg) translate(4px,4px)":"none", transition:"transform 0.2s, background 0.15s" }} />
              <div style={{ width:14, height:2, background: drawerOpen?"#2a8a5a":T.text, borderRadius:2, opacity: drawerOpen?0:1, transition:"opacity 0.15s, background 0.15s" }} />
              <div style={{ width:18, height:2, background: drawerOpen?"#2a8a5a":T.text, borderRadius:2, transform: drawerOpen?"rotate(-45deg) translate(4px,-4px)":"none", transition:"transform 0.2s, background 0.15s" }} />
            </button>
          </div>
        </div>
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", zIndex:599, backdropFilter:"blur(4px)" }} onClick={()=>setDrawerOpen(false)}>
      <div style={{ position:"fixed", top:0, right:0, bottom:0, width:"280px", background:T.drawerBg, borderLeft:`1px solid ${T.drawerBorder}`, zIndex:600, display:"flex", flexDirection:"column", boxShadow:T.dark?"none":"-8px 0 32px rgba(30,40,80,0.1)", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ padding:"14px 16px", flex:1, display:"flex", flexDirection:"column", gap:"4px" }}>
          ...
          {/* Nav items */}
          {[
            {icon:"search",label:"Browse Profiles",action:()=>{setDrawerOpen(false);setPage("search");setViewingProfile(null);}},
            isLoggedIn&&{icon:"person",label:"My Profile",action:()=>{setDrawerOpen(false);setPage("profile");setViewingProfile(null);}},
            isLoggedIn&&{icon:"explore",label:"Explore Profiles",action:()=>{setDrawerOpen(false);setPage("search");setViewingProfile(null);}},
            {icon:"chat",label:"Messages",action:()=>{setDrawerOpen(false);if(!isLoggedIn){setPage("login");}else{showToast("Opening messages…");}}},
            {icon:"grade",label:"Top Profiles",action:()=>{setDrawerOpen(false);showToast("Loading top profiles…");}},
            {icon:"share",label:"Share My Profile",action:()=>{setDrawerOpen(false);showToast("Opening share sheet…");}},
      ...
      <div style={{ width:"100%", maxWidth:"420px" }}>
        ...
        <div style={{ background:T.cardBg, borderRadius:"24px", padding:"22px", marginBottom:"14px", border:`1px solid ${T.cardBorder}`, boxShadow:T.cardShadow, position:"relative" }}>
          ...
          <div style={{ display:"flex", borderTop:`1px solid ${T.divider}`, paddingTop:"14px" }}>
            {[{n:fmtN(12500),l:"Followers",action:()=>setPeopleModal({title:"Followers · 12.5k",ids:FOLLOWERS_IDS})},{n:"320",l:"Following",action:()=>setPeopleModal({title:"Following · 320",ids:FOLLOWING_IDS})},{n:String(portfolioPhotos.length||links.length||0),l:"Portfolio",action:null}].map((s,i)=>(
              ...
              {links.map(link=>{
                ...
                return (
                  <div key={link.uid} style={{ position:"relative" }}>
                    ...
                    {/* Social links open preview modal first */}
                    <div onClick={!editMode ? ()=>setPreviewLink(link) : undefined} style={{ cursor: editMode?"default":"pointer" }}>
                      <div style={{ display:"flex",alignItems:"center",gap:"15px",padding:"14px 16px",borderRadius:"16px",background:T.linkCard,border:`1px solid ${editMode?"rgba(220,38,38,0.25)":T.linkBorder}`,boxShadow:T.dark?"none":"0 1px 8px rgba(30,40,80,0.06)" }}
                        ...
                        ):<MI name="open_in_new" size={18} color={plat.color} style={{ flexShrink:0, opacity:0.85 }} />}
                      </div>
                    </div>
                  </div>
                );
  ...
  const Footer = () => (
    <footer style={{ background:T.footerBg,borderTop:`1px solid ${T.footerBorder}`,padding:"28px 24px 24px",transition:"background 0.25s" }}>
      <div style={{ display:"flex",alignItems:"center",gap:"8px",marginBottom:"4px" }}>
        ...
        <span style={{ fontSize:"11px",color:T.mutedText }}>Simple. Social. Yours.</span>
      </div>
      <div style={{ fontSize:"12px",color:T.subText,marginBottom:"20px" }}>Your link in bio. All your socials. Start for free. 🌍</div>
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"2px 16px",marginBottom:"20px" }}>
        {[{icon:"search",label:"Browse Profiles",fn:()=>{setPage("search");setViewingProfile(null);}},{icon:"login",label:"Login / Sign Up",fn:()=>setPage("login")},{icon:"person",label:"My Profile",fn:()=>{if(isLoggedIn){setPage("profile");}else setPage("login");}},{icon:"chat",label:"Messages",fn:()=>{}},{icon:"grade",label:"Top Profiles",fn:()=>{}},{icon:"share",label:"Share My Profile",fn:()=>{}},{icon:"security",label:"Safety Centre",fn:()=>{}},{icon:"description",label:"Terms of Use",fn:()=>{}},{icon:"policy",label:"Privacy Policy",fn:()=>{}},{icon:"info",label:"About UmLinked",fn:()=>{}}].map(({icon,label,fn})=>(
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
        <span style={{ fontSize:"11px",color:T.mutedText }}>© 2025 UmLinked</span>
        <span style={{ fontSize:"11px",color:"#2a8a5a",background:T.tagBg,padding:"3px 10px",borderRadius:"20px",border:`1px solid ${T.tagBorder}` }}>🌍 For Everyone</span>
      </div>
    </footer>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.bodyBg, fontFamily:"'DM Sans','Segoe UI',sans-serif", color:T.text, transition:"background 0.25s,color 0.25s" }}>
      ...
      {/* MiniProfile renders inline below Nav — hamburger stays fully accessible */}
      {viewingProfile ? (
        <MiniProfile person={viewingProfile} onBack={()=>setViewingProfile(null)} onAuthGate={handleAuthGate} isLoggedIn={isLoggedIn} T={T} />
      ) : (
        <>
          {page==="search" && <SearchPage onViewProfile={p=>{setViewingProfile(p);}} onGoLogin={()=>setPage("login")} T={T} />}
          {page==="profile" && isLoggedIn && <MyProfilePage />}
          {page==="login" && <div><LoginPage /></div>}
          {page!=="login" && <Footer />}
        </>
      )}
      ...
      {previewLink && <SocialLinkPreviewModal link={previewLink} onClose={()=>setPreviewLink(null)} T={T} />}
